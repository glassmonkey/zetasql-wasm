...
// TestLanguageOptions_EnableLanguageFeature verifies that calling
// EnableLanguageFeature mutates the Features map. The full struct is
// compared so any extraneous mutation surfaces.
func TestLanguageOptions_EnableLanguageFeature(t *testing.T) {
	tests := []struct {
		name    string
		enable  []generated.LanguageFeature
		want    *LanguageOptions
	}{
		{
			name:   "enable single feature",
			enable: []generated.LanguageFeature{generated.LanguageFeature_FEATURE_ANALYTIC_FUNCTIONS},
			want: &LanguageOptions{
		...
		{
			name: "enable two features",
			enable: []generated.LanguageFeature{
				generated.LanguageFeature_FEATURE_ANALYTIC_FUNCTIONS,
				generated.LanguageFeature_FEATURE_TABLESAMPLE,
			},
			want: &LanguageOptions{
				Features: map[generated.LanguageFeature]bool{
					generated.LanguageFeature_FEATURE_ANALYTIC_FUNCTIONS: true,
					generated.LanguageFeature_FEATURE_TABLESAMPLE:        true,
				},
				Keywords: map[string]bool{},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, f := range tt.enable {
				sut.EnableLanguageFeature(f)
			}
...
// TestLanguageOptions_DisableAllLanguageFeatures verifies that previously
// enabled features are cleared.
func TestLanguageOptions_DisableAllLanguageFeatures(t *testing.T) {
	// Arrange
	sut := NewLanguageOptions()
	sut.EnableLanguageFeature(generated.LanguageFeature_FEATURE_ANALYTIC_FUNCTIONS)
	sut.EnableLanguageFeature(generated.LanguageFeature_FEATURE_TABLESAMPLE)

	// Act
	sut.DisableAllLanguageFeatures()
	got := sut
